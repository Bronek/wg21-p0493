...
to function naming necessitated by the fact that `max` and `min` do not exist as infix operators.
...
- Following LEWG feedback, revert to require store if the value does not change __TODO__
- Rebase on draft [@N4901] __TODO__
- Add floating numbers support to wording, consistently with [@P0020] __TODO__
- Modernize remaining motivating example __TODO__
- Add example implementation based on CAS loop __TODO__
