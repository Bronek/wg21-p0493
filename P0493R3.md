...
* `bench1` is populating a fixed size queue, using the [@Gong] algorithm
  presented above. We have decided to **drop** the results from this benchmark
  due to excessive standard deviation of results. We attribute this standard
  deviation to the CAS operation of the algorithm main loop (looking for the
  next free entry), which is inherently non-deterministic and dominates the
  algorithm execution time.
* `bench2` is finding a maximum value from a PRNG. We were able to achieve
...
The results presented below are from `bench2`. We have measured the nanosecond
time of two different implementations (one CAS-loop and another in hardware) of
